...
shock_scenarios = {"FVX": shock_fvx / 100, "SP500": shock_sp500 / 100}

scenario_type = st.sidebar.selectbox("Scenario Type", ["Mild", "Severe", "Historical"])
...
if uploaded_file is not None:
    ...
    st.subheader(f"📈 Predicted VaR under {scenario_type} Scenario")

    if scenario_type == "Mild":
        fvxScenarios = np.arange(-0.01, 0.01, 0.005)
        spScenarios = np.arange(-0.01, 0.01, 0.005)
    elif scenario_type == "Severe":
        fvxScenarios = np.arange(-0.05, 0.05, 0.01)
        spScenarios = np.arange(-0.05, 0.05, 0.01)
    else:
        fvxScenarios = returns["FVX"].unique()
        spScenarios = returns["SP500"].unique()

    scenarios = []
    for fvx_val in fvxScenarios:
        for sp_val in spScenarios:
            row = [fvx_val, sp_val]
            for stock in stockNames:
                alpha = float(modelCoeffs.loc[modelCoeffs["Names"] == stock, "Intercept"])
                beta_sp = float(modelCoeffs.loc[modelCoeffs["Names"] == stock, "SP500"])
                beta_fvx = float(modelCoeffs.loc[modelCoeffs["Names"] == stock, "FVX"])
                pred = alpha + beta_sp * sp_val + beta_fvx * fvx_val
                row.append(pred)
            scenarios.append(row)

    scenario_df = pd.DataFrame(scenarios, columns=["FVX", "SP500"] + stockNames)
    scenario_cov = scenario_df[stockNames].cov()
    scenario_risk = np.dot(np.dot(weights, scenario_cov), weights.T)
    scenario_var = calculateVaR(scenario_risk, confLevel)

    st.metric(label="Scenario-Based VaR (%)", value=f"{scenario_var:.4f}")

    st.write("📉 Heatmap of Average Predicted Returns")
    mean_returns = scenario_df[stockNames].mean(axis=1)
    heatmap_data = mean_returns.values.reshape(len(fvxScenarios), len(spScenarios))

    fig2, ax2 = plt.subplots(figsize=(10, 6))
    sns.heatmap(heatmap_data, xticklabels=np.round(spScenarios, 2), yticklabels=np.round(fvxScenarios, 2), cmap="coolwarm", ax=ax2)
    ax2.set_xlabel("SP500 Shock")
    ax2.set_ylabel("FVX Shock")
    st.pyplot(fig2)
